...
covariances <- function(t, state, parameters) {
  with(as.list(c(state,parameters)), {
    ...
    dV2 <- 2*lambda_plus*V1 + (alpha_plus+beta_plus)*M1 + 2*omega_plus*C + omega_plus*M2
...
out_df %>% rename(c("<Z->" = "M1","<Z+>" = "M2")) %>% melt(value.name = "Z", id = "t") %>% ggplot(aes(t,Z, col=variable)) + ylab("Z") + geom_line() +
